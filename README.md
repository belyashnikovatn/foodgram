...
|Определить модель для пользователя|Выполнено| Поля пользователя стандартные -> модель User не переписываем, используем стандартную |
|Определить приложения|Выполнено| 2 приложения: recepie, api |
|Определить список функций для проектирования вьюсетов|Очередь| --- |
|Создать модели данных|Очередь| --- |
